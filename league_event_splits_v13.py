...
Version 13: PPDA numerator
--------------------------
Adds a second count of passes allowed, this one including crosses, throw-ins and
keeper throws. PPDA measures passes a team failed to stop, and a cross you
allowed is still a pass you allowed, so their numerator probably counts the lot.
Both are written, so the two can be compared against the published figure.

Version 12: measured against Opta's published figures
-----------------------------------------------------
Five corrections, each with a published number to aim at.

* Own goals are no longer counted as shots. Opta exclude them, which is why our
  shot totals sat one high at both ends.
* Conversion is total goals over all shots, including blocked ones. Their page
  says so outright, which settles a contradiction between two Opta documents.
* Final-third passes count passes that END in the final third, not those that
  start there. Ours came to 772 against their 958.
* PPDA had the zone inverted. The rule is opposition passes outside OUR
  defensive third, which in their coordinates is the opposite end of the pitch.
* Sequences no longer break on a team's own defensive touches, and now begin
  where possession is won. Ours were fragmenting badly: 4.37 passes per sequence
  published against 3.37 counted, which threw off everything downstream.

Version 11: sequences, phase 4
------------------------------
Adds Opta's sequence measures, worked out from the timestamps and coordinates
already in the cache.

A sequence is a passage of play belonging to one team, ended by a defensive
action, a stoppage, or a shot. Each sequence's numbers are recorded on its first
event, so they split by time block and game state like everything else.

* Build-up attacks: open-play sequences of ten or more passes ending in a shot
  or a touch in the box.
* Direct attacks: open-play sequences starting in a team's own half that move at
  least half their distance towards goal and end in a shot or a box touch.
* High turnovers: sequences starting within 40 metres of the opponent's goal,
  and how many end in a shot.
* Pressed sequences: opponent passages starting in their own third, three passes
  or fewer, ending in their own half. Counted from the other side, so a team's
  pressing shows up in the against column.
* Sequence time, passes, progress, length and start distance, which give direct
  speed and how high a team begins its attacks.
...
# Pitch size in metres, for turning grid units into distances.
PITCH_LENGTH_M = 105.0
PITCH_WIDTH_M = 68.0

# Sequence rules.
BUILD_UP_PASSES = 10          # passes that make a sequence a build-up attack
DIRECT_PROGRESS_SHARE = 0.5   # share of movement that must be towards goal
HIGH_TURNOVER_METRES = 40.0   # from the opponent's goal
PRESSED_MAX_PASSES = 3

ON_BALL = {"Pass", "TakeOn", "BallTouch", "Dribble", "Carry", "KeeperPickup",
           "Goal", "MissedShots", "SavedShot", "ShotOnPost"}
DEFENSIVE = {"Tackle", "Interception", "Clearance", "BlockedPass", "BallRecovery",
             "Aerial", "Challenge", "Smother", "Save", "Punch", "Claim",
             "CrossNotClaimed", "Dispossessed"}
# Winning the ball starts a passage of play for the team that wins it.
BALL_WINNING = {"Tackle", "Interception", "BallRecovery", "Smother", "Save",
                "Claim", "BlockedPass"}
STOPPAGE = {"Foul", "Out", "CornerAwarded", "OffsidePass", "OffsideGiven",
            "OffsideProvoked", "Card", "SubstitutionOn", "SubstitutionOff",
            "Start", "End", "FormationChange", "FormationSet", "Error",
            "ShieldBallOpp", "PenaltyFaced"}
SET_PIECE_START = ("CornerTaken", "FreekickTaken", "ThrowIn", "GoalKick")
...
def build_sequences(ev: pd.DataFrame) -> pd.DataFrame:
    """Split a match into sequences and measure each one.

    Returns one column per sequence measure, with the value sitting on the
    sequence's first event and zero everywhere else. That way the existing
    time-block and game-state grouping picks them up untouched.
    """
    n = len(ev)
    zeros = lambda: np.zeros(n, dtype=float)
    out = {name: zeros() for name in
           ["sequences", "sequence_passes", "sequence_seconds", "sequence_progress_m",
            "sequence_length_m", "sequence_start_x_m", "build_up_attacks",
            "direct_attacks", "high_turnovers", "high_turnover_shots",
            "sequence_pressed", "shot_ending_sequences"]}

    types = ev["type"].to_numpy()
    teams = ev["team"].to_numpy()
    periods = ev["period"].astype(str).to_numpy()
    minutes = pd.to_numeric(ev["minute"], errors="coerce").fillna(0).to_numpy()
    seconds = pd.to_numeric(ev["second"], errors="coerce").fillna(0).to_numpy()
    xs = pd.to_numeric(ev["x"], errors="coerce").to_numpy()
    ys = pd.to_numeric(ev["y"], errors="coerce").to_numpy()
    quals = ev["_qset"].to_numpy()
    outcomes = ev["outcome_type"].astype(str).to_numpy()

    current: list[int] = []          # row positions in the open sequence
    current_team = None
    current_period = None
    won_here = [False] * n           # did this sequence begin by winning the ball

    def close() -> None:
        nonlocal current, current_team
        if current:
            measure(current)
        current, current_team = [], None

    def measure(rows: list[int]) -> None:
        first, last = rows[0], rows[-1]
        if np.isnan(xs[first]):
            return
        # Winning the ball and losing it again before touching it is not a
        # passage of play. Opta count possession, so require at least one
        # on-ball event.
        if not any(types[r] in ON_BALL for r in rows):
            return
        passes = sum(1 for r in rows if types[r] == "Pass")
        t0 = minutes[first] * 60 + seconds[first]
        t1 = minutes[last] * 60 + seconds[last]

        # Where the ball started and how far it moved towards goal.
        end_x = xs[last]
        progress = (end_x - xs[first]) / 100 * PITCH_LENGTH_M
        length = 0.0
        prev = first
        for r in rows[1:]:
            if np.isnan(xs[r]) or np.isnan(xs[prev]):
                continue
            dx = (xs[r] - xs[prev]) / 100 * PITCH_LENGTH_M
            dy = (ys[r] - ys[prev]) / 100 * PITCH_WIDTH_M
            length += float(np.hypot(dx, dy))
            prev = r

        open_play = not any(has_q(quals[first], *SET_PIECE_START) for _ in (0,))
        ends_in_shot = types[last] in SHOT_TYPES
        touched_box = any(
            (not np.isnan(xs[r])) and xs[r] >= BOX_X_MIN
            and BOX_Y_MIN <= ys[r] <= BOX_Y_MAX for r in rows)
        reached_attack = ends_in_shot or touched_box

        out["sequences"][first] = 1
        ball_won = won_here[first]
        out["sequence_passes"][first] = passes
        out["sequence_seconds"][first] = max(t1 - t0, 0)
        out["sequence_progress_m"][first] = progress
        out["sequence_length_m"][first] = length
        out["sequence_start_x_m"][first] = xs[first] / 100 * PITCH_LENGTH_M
        if ends_in_shot:
            out["shot_ending_sequences"][first] = 1

        if open_play:
            if passes >= BUILD_UP_PASSES and reached_attack:
                out["build_up_attacks"][first] = 1
            if (xs[first] < 50 and reached_attack and length > 0
                    and progress / length >= DIRECT_PROGRESS_SHARE):
                out["direct_attacks"][first] = 1
            # A high turnover is the ball being WON within 40 metres of the
            # opponent's goal, not merely a passage that happens to start there.
            if (ball_won
                    and xs[first] >= 100 - (HIGH_TURNOVER_METRES / PITCH_LENGTH_M * 100)):
                out["high_turnovers"][first] = 1
                if ends_in_shot:
                    out["high_turnover_shots"][first] = 1
            # Pressed: began deep, went nowhere. Counted here for the team that
            # was pressed, so it appears in their opponent's against column.
            if (xs[first] < OWN_THIRD and passes <= PRESSED_MAX_PASSES
                    and not np.isnan(xs[last]) and xs[last] < 50):
                out["sequence_pressed"][first] = 1

    for i in range(n):
        kind, team, period = types[i], teams[i], periods[i]
        if period != current_period:
            close()
            current_period = period

        if kind in STOPPAGE or pd.isna(team):
            close()
        elif kind in SHOT_TYPES:
            if current and team == current_team:
                current.append(i)
            else:
                close()
                current, current_team = [i], team
            close()
        elif kind in ON_BALL:
            if not current or team != current_team:
                close()
                current, current_team = [i], team
            else:
                current.append(i)
        elif kind in DEFENSIVE:
            # A team's own defensive touch does not end its own passage. Nor does
            # an opponent trying and failing: losing an aerial, missing a tackle
            # or being beaten leaves the ball where it was.
            failed = (kind == "Challenge"
                      or (kind in {"Aerial", "Tackle"}
                          and outcomes[i] != "Successful"))
            if current and team != current_team and not failed:
                close()
            if (not current and kind in BALL_WINNING and not pd.isna(team)
                    and outcomes[i] == "Successful"):
                # Possession begins where it is won.
                current, current_team = [i], team
                won_here[i] = True
    close()
...
def build_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    in_final_third = df["end_x"].ge(FINAL_THIRD)          # where the pass arrives
    into_final_third = in_final_third & ~from_final_third  # and crossed the line
    outside_own_third = df["x"].ge(OWN_THIRD)
    # For PPDA: passes played outside the pressing team's defensive third, which
    # in the passing team's own coordinates is everything below their final third.
    outside_opponent_defensive_third = df["x"].lt(FINAL_THIRD)
    ...
    # An own goal is not a shot. Opta exclude them from shots, and so do we.
    # They appear to keep own goals inside their on-target total even so, which
    # is why our on-target can read one lower than theirs for a club that has
    # scored one. We leave it out of both, so shots still equal on target plus
    # off target plus blocked.
    shot = t.isin(SHOT_TYPES) & ~is_own_goal
    ...
    on_target = (t.eq("SavedShot") & ~is_blocked) | scored
    ...
    return pd.DataFrame({
        ...
        "passes_final_third": (is_pass & in_final_third).astype(int),
        "passes_final_third_completed": (is_pass & in_final_third & ok).astype(int),
        "passes_from_final_third": (is_pass & from_final_third).astype(int),
        ...
        "passes_outside_own_third": (is_pass
                                     & outside_opponent_defensive_third).astype(int),
        # Same zone, but every delivery: crosses, throw-ins and keeper throws too.
        "deliveries_outside_own_third": (pass_event
                                         & outside_opponent_defensive_third).astype(int),
...
def summarise_match(path: Path) -> tuple[pd.DataFrame, pd.DataFrame] | None:
    ...
    flags = pd.concat([flags, build_sequences(ev)], axis=1)
...
def add_rates(df: pd.DataFrame, suffix: str) -> pd.DataFrame:
    ...
    df[f"final_third_pass_accuracy_pct{suffix}"] = pct("passes_final_third_completed",
                                                       "passes_final_third")
    # Their page states that conversion includes blocked shots, and their figure
    # works out as total goals over all shots, own goals included.
    df[f"conversion_pct{suffix}"] = pct("goals", "shots")
    df[f"shot_conversion_pct{suffix}"] = pct("goals_from_shots", "shots")
