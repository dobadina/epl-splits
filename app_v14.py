...
METRICS = {
    ...
    "on_target_pct": ("Shooting accuracy %", False, 1,
                      ...
                      "is how Opta defines it. A block says nothing about whether "
                      "the shot was on target. Our on-target count can sit one "
                      "below theirs for a club that has scored an own goal: they "
                      "appear to keep it in on-target while leaving it out of "
                      "shots, and we leave it out of both so the figures still "
                      "add up."),
    ...
    "conversion_pct": ("Conversion %", False, 1,
                       "Goals per shot, counting blocked shots and own goals, "
                       "which is how Opta publish it."),
    "shot_conversion_pct": ("Conversion from own shots %", False, 1,
                            "Goals the team actually shot, per shot. Strips out "
                            "own goals the opposition scored."),
...
def derive(df: pd.DataFrame, rank: bool = True) -> pd.DataFrame:
    ...
    # Opta's conversion: total goals over all shots, blocked ones included.
    # Their stats page states this outright.
    d["conversion_pct"] = div(goals, shots) * 100
    d["shot_conversion_pct"] = div(shot_goals, shots) * 100
    ...
    # PPDA counts passes a team failed to stop, so the numerator includes every
    # delivery: crosses, throw-ins and keeper throws as well as ordinary passes.
    # The denominator is Opta's documented list of defensive actions. Tested
    # against all twenty clubs' published figures, this fits to 3.7% on average.
    press_zone = col(d, "pressing_actions_outside_own_third_for")
    allowed = col(d, "deliveries_outside_own_third_against")
    if allowed.isna().all():
