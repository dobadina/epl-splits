...
METRICS = {
    ...
    # Finishing
    "on_target_pct": ("On target %", False, 1, "Share of shots that reach the keeper."),
    "blocked_share_pct": ("Shots blocked %", True, 1,
                          "Share of attempts that hit a defender before the keeper."),
    "conversion_pct": ("Conversion %", False, 1, "Goals per shot."),
    ...
    "pass_accuracy_pct": ("Pass accuracy %", False, 1, "Completed passes."),

    # Duels
    "duel_win_pct": ("Duel win %", False, 1, "Aerial and ground duels combined."),
...
def derive(df: pd.DataFrame, rank: bool = True) -> pd.DataFrame:
    ...
    # Finishing
    d["on_target_pct"] = div(sot, shots) * 100
    d["blocked_share_pct"] = div(blocked, shots) * 100
    d["conversion_pct"] = div(goals, shots) * 100
    d["goals_per_sot_pct"] = div(goals, sot) * 100
    d["shots_per_goal"] = div(shots, goals)
    ...
    d["goals_per_sot_faced_pct"] = div(conceded, sot_faced) * 100
    d["shots_faced_per_goal"] = div(faced, conceded)
