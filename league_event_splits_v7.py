...
def build_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ground_duel = take_on | tackle | challenge | dispossessed
    ...
    return pd.DataFrame({
        ...
        "crosses_attempted": (is_pass & is_cross & ~is_corner & ~dead_ball).astype(int),
        "crosses_completed": (is_pass & is_cross & ~is_corner & ~dead_ball & ok).astype(int),
        ...
        "corners": (is_pass & is_corner).astype(int),
        "chances_created": (is_pass & (is_key_pass | is_assist)).astype(int),
        "assists": (is_pass & is_assist).astype(int),
        ...
        "ground_duels_won": ((take_on & ok) | (tackle & ok)).astype(int),
        "duels_contested": (ground_duel | aerial).astype(int),
        "duels_won": ((take_on & ok) | (tackle & ok) | (aerial & ok)).astype(int),
